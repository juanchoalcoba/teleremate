  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const requestBody = {
        ...
        paymentMethod: paymentMethod, // deposit or mercadopago
      };
      ...
      const result = await mutation.mutateAsync(requestBody);

      if (paymentMethod === "mercadopago" && result.data?.init_point) {
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="flex gap-3 pt-4 mt-6">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className={`flex-1 px-4 py-3 text-white font-semibold rounded-lg hover:shadow-lg transition disabled:opacity-50 flex items-center justify-center gap-2 ${paymentMethod === 'mercadopago' ? 'bg-[#009ee3] hover:bg-[#008cce]' : 'bg-gradient-to-r from-green-500 to-green-600'}`}
              >
                {isLoading ? (
                  <>
                  ...
                  paymentMethod === 'mercadopago' ? 'Pagar con MercadoPago' : 'Confirmar Compra'
                )}
              </button>
            </div>
          </form>
